...
#%%
    ...
    im=ax.pcolormesh(lats,alts,sr512.T,cmap=cmap,shading='auto',vmin=0,vmax=vmax)
    # add the vortex center and the contour of mid vorticity
    if vortex:
        ax.plot(latv,zv,'firebrick',marker='+',ms=21,mew=3)
        ax.contour(latcut,zcut,VOcut,levels=[0.5*vomax,],colors='white',linewidths=2)
    # set limits of the plot
    ax.set_xlim(latmin,latmax)
    ax.set_ylim(yinf,ysup)
    if lonmid>180:
        ax.set_title(date.strftime('%d %b   ')+str(int(360-lonmid))+'W')
    else:
        ax.set_title(date.strftime('%d %b   ')+str(int(lonmid))+'E')
    ...
    plt.savefig(join(tmp_dir,'CALIOP_BS_{}_{}.jpg'.format(sel,i)),bbox_inches='tight')
