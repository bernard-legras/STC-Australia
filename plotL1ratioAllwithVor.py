...
for i in listi:
    ...
    if vertical:
        ...
        #fig, (ax0, ax1) = plt.subplots(1, 2, gridspec_kw={'height_ratios': [3, 1.5]})
        ...
        if vortex:
            cm_lon = 0
            if datr.attr['lons'][-1] > 180: cm_lon=180
            projplate = ccrs.PlateCarree(central_longitude=cm_lon)
            ax1 = fig.add_subplot(gs[1,:],projection = projplate)
            ax1=datr.show('VO',scale=1.e5,lev = trac['kz'][iv],show=False,axf=ax1,xylim=True,
                      figsize=None,txt = '',aspect=1.2,clim=(-1,8))
        ...
        plt.savefig(join(tmp_dir,'VOV_CALIOP_BS_{}_{}.jpg'.format(sel,i)),dpi=300,bbox_inches='tight')
        plt.savefig(join(tmp_dir,'VOV_CALIOP_BS_{}_{}.png'.format(sel,i)),dpi=300,bbox_inches='tight')
