...
for i in range(388):
    ...
    #lower = 65
    #upper= 25
    # handling chnage in vertical sampling
    upper = 0
    lower = 30
    if date >= datetime(2020,2,1,0):
        upper = 0
        lower = 20
    var = 'O3prime'
    var = 'LPV'
    ...
    try:
        ...
        upper = max(trac['kz'][-1] - 2,0)
        if date >= datetime(2020,2,1,0): lower = min(20,trac['kz'][-1] + 3)
        else: lower = min(30,trac['kz'][-1] + 3)
    except: pass
    if date == datetime(2020,2,1,0):
        upper += 12
        lower += 12
        lower = min(lower,20)
        if upper == 20: upper = 19
    if var == 'VO':
        pass
    if var == 'LPV':
        if i == 326:
    ...
    if var == 'O3prime':
        if 217 <= i <= 225: lat = -37
        if 230 <= i <= 235: lat = -37
        if i >= 251: lower = min(lower,9)
        if i == 258: lon = -55
        if i == 259: lon = -58
        if i == 260: lon = -59
        if i >= 260: lower = min(lower,7)
        if i == 283: lon = -130
        if i == 287:
            lon = -141
            idx = 1
...
# The tracking is truncated at i = 330, that is 26 March 2020 when the vortex has
# returned to Australia
if var=='LPV':
    trac_PV = {}
    for k in trac.keys():
        trac_PV[k] = trac[k][:330].copy()
    trac1 = trac_PV
elif var == 'O3prime':
    trac_O3 = {}
    for k in trac.keys():
        trac_O3[k] = trac[k][:330].copy()
    trac2 = trac_O3
elif var == 'VO':
    trac_VO = {}
    for k in trac.keys():
        trac_VO[k] = trac[k][:330].copy()
    trac3 = trac_VO
...
pickle.dump([trac_VO,trac_PV,trac_O3],open('Vortex-track_1stVortex_ERA5PV.pkl','wb'))
...
fig.suptitle('Australian 2020 event   main vortex tracking')
fig.autofmt_xdate()
if figsav:
    plt.savefig(join('figs','VortexMotion_1stVortex_ERA5PV.png'),**figargs)
plt.show()
