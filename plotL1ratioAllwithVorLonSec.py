...
The vorticity and CALIOP images share teh same verical axis.
...
def get_vortex_map(iv,lonv,latv,c1,c2):
    ...
    if ((lo1 < 0) & (lo2 < 0)):
        ...
        dats = dat1
    datr = dats.extract(latRange=(la1,la2),lonRange=(lo1,lo2),varss=['VO','Z'])
    return datr
...
#%%
    if horizontal:
        fig, (ax0, ax1) = plt.subplots(ncols=2,sharey=True,figsize=(15,6))
        ...
        if lonmid>180:
            ax0.set_title(date.strftime('%d %b   ')+str(int(360-lonmid))+'W',fontsize=fs)
        else:
            ax0.set_title(date.strftime('%d %b   ')+str(int(lonmid))+'E',fontsize=fs)
        ...
        if vortex:
            ax1=datr.chartlonz('VO',trac['lats'][iv],show=False,scale=1.e5,clim=(-1,vormax),levs=(20,80),
                               txt='',axf=ax1,ylabel=False)

            #ax1.plot(lonv-cm_lon,latv,'firebrick',marker='+',markersize=100)
            ax1.set_title(date.strftime('%d %b    ')+str(int(-trac['lats'][iv]))+'S',
                          fontsize=fs)
        ...
        fig.suptitle(u'Lidar backscatter ratio and vorticity (10$^{-5}$ s$^{-1}$)',fontsize=18)
